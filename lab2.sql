-- 2.1
-- Замени в примере ниже table1 на любое имя таблицы из твоей базы данных из лабораторной работы №1 и изучи данные в этой таблице, чтобы вместе с Алексом продвигаться дальше по изучению SQL:
SELECT *
FROM film_actor;
...
-- В задании был пример, представленный ниже. Какой результат ты получил? Как работает этот запрос и почему? В пояснении отдельно опиши, что делают SELECT, FROM и WHERE.
SELECT concat(cust.last_name, ', ', cust.first_name)
full_name
FROM
(SELECT first_name, last_name, email
...
-- Напиши запрос к таблице rental, который возвращает идентификаторы клиентов, бравших фильмы напрокат 5 июля 2005 года. Выведи по одной строке для каждого уникального идентификатора клиента.
